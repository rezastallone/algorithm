...
fun main() {
    val islands = connectedisland.generateIslands()
    val largestIsland = connectedisland.getNumberOfIsland(islands)
...
fun getLargestIsland(islands: MutableList<MutableList<Int>>): Int {
    ...
    for ( row in 0 until  islands.size ){
        ...
        for ( col in 0 until rowIslands.size ){
            if ( rowIslands[col] == 1 ){
                largestIsland = Math.max(connectedisland.calculateNumberOfIsland(row, col, islands), largestIsland)
            }
        }
        println()
    }
...
fun calculateLargestIsland(row: Int, col: Int, islands: MutableList<MutableList<Int>>): Int {
    ...
    for ( rowItr in row - 1 .. row + 1){
        for ( colItr in col - 1 .. col + 1){
            size += connectedisland.calculateNumberOfIsland(rowItr, colItr, islands)
        }
    }
    return size
}
